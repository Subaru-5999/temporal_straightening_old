...
## 23. **THE ROOT CAUSE, QUANTIFIED** — GD optimizes 54% below the model's own error floor

This is the strongest and most defensible result the project has produced. It is a property of the
**objective**, measured in the model's own units, on the paper's own checkpoint and protocol.

### 23.1 The numbers

PushT, matched straightened baseline, open-loop, seed 100, 50 tasks, `alpha=1`, oracle
initialization (`gt_actions` — privileged, diagnosis only):

| opt_steps | success | obj_init | obj_final | obj drop | pathdev_init | pathdev_final |
|---|---|---|---|---|---|---|
| 0 | **1.00** | 0.031905 | 0.031905 | 0.0% | 0.3981 | 0.3981 |
| 25 | 0.84 | 0.031905 | 0.016355 | **48.7%** | 0.3981 | 0.3952 |
| 100 | 0.86 | 0.031905 | 0.014642 | **54.1%** | 0.3981 | 0.3951 |

### 23.2 Reading 1 — the true goal is not the argmin, and it is not close

`obj_init = 0.0319` is the model's cost for a plan whose **true final state distance is exactly
0.0**. GD finds plans at `0.0146` — **less than half** the cost of the exact solution — and those
plans are worse. So the objective's minimizer is not the task solution, and the gap is a factor of
2.2 in the model's own units, not a rounding error. No encoder-side regularizer changes this,
which is a coherent account of why five representation-side extensions produced no win (§22.1).

### 23.3 Reading 2 — the mechanism: GD optimizes below the objective's NOISE FLOOR

The deeper point is *why* `obj_init` is 0.0319 and not ~0. The goal image is generated by rolling
the true actions in the env, so a perfect model would predict the goal latent exactly and score ~0.
The 0.0319 residual **is the model's own H=5 prediction error.**

Consistency check with §20's independent diagnostic: rollout error on real actions was
`k1..k4 = 0.004485 / 0.006510 / 0.008630 / 0.011138`, so an H=5 extrapolation is ~0.013–0.014 in
that weighting. Different normalization (that measures visual+proprio jointly; the objective uses
`visual + alpha*proprio`), but the same order of magnitude. The two measurements agree.

So: **the objective has a noise floor at roughly the model's multi-step prediction error, and GD
drives the objective 54% below it.** Below the floor the objective carries no information about
plan quality — differences at that resolution are model error, not task progress. GD spends the
back half of its optimization budget fitting the model's own mistakes. This is textbook model
exploitation, now with a number attached.

### 23.4 Reading 3 — path shape is CONFIRMED irrelevant (the corridor dies twice)

`pathdev` goes 0.3981 → 0.3951. GD's exploited plans are **not** off-manifold in the path-shape
sense; they are indistinguishable from the exact solution on that statistic (0.395 vs 0.398).
This independently re-kills the corridor (§22.10) using the "measure, don't guess" protocol, and
it tells us the exploitation happens at the **terminal latent**, not along the trajectory: GD lands
`ẑ_H` nearer `z_g` in latent while the true state sits ~19 units away. That is **latent aliasing**
near the goal — distinct true states mapping to nearby latents — and curvature regularization does
not address it.

### 23.5 What follows, and the one thing that must be measured first

The natural fix is the **discrepancy principle** from inverse problems (Morozov): when the forward
operator is only accurate to δ, do not drive the residual below δ, because past that point you are
fitting noise. Here δ² is the objective's noise floor, and it is estimable **without any privileged
information** — real recorded actions on training/validation trajectories, no test tasks, no
rewards, no success labels. `diagnose_planning.py` now measures exactly that (`objective_floor`),
in the exact units of `objective_fn_last`.

**BUT the zero-init sweep (§22.17) is a hard constraint on this idea.** From zero init success was
*monotone* in opt_steps — 0.52 at 50 steps, 0.72 at 100 — so stopping earlier from a zero start
HURTS. Any floor-based stopping rule has to be reconciled with that. Two possibilities, and the
data cannot yet distinguish them:
1. From zero init GD never gets near the floor, so a floor-triggered stop never fires and the rule
   is a no-op there while still helping in the good-initialization regime.
2. It does cross the floor early, in which case the rule would stop at 50 steps and *cost* 20
   points, and the whole idea is dead.

**Required next measurement (cheap, ~10 min): the zero-init sweep WITH the obj values**, which the
logging bug (§22.17) prevented us from capturing. That single table decides between 1 and 2. Do not
implement any stopping rule before it.

### 23.6 Honest scope of the result

- Single seed (100), single checkpoint. Confirm on seeds 200/300 before any claim.
- The oracle-init arms use `gt_actions` = privileged information. Valid as diagnosis, never as a
